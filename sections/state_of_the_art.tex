...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=2cm]
        ...
        \node (dc) [startstop, below of=start] {Data pre-processing};
        \node (dpr) [startstop, below of=start] {Algorithmic intervention to inject bias addresssing in ML algorithms};
        \node (mtpe) [startstop, below of=start] {Continuous monitoring};
        \draw [arrow] (start) -- (dc);
        \draw [arrow] (start) -- (dpr);
        \draw [arrow] (start) -- (mtpe);
    \end{tikzpicture}
