...
\label{section:workflow-principles}

Here is reported the set of principles that led our design and development of the Fair-by-Design (FbD) workflow, as highlighted in \cref{fig:workflow_principles}

\begin{figure}
    \centering
    \begin{tikzpicture}[node distance=2cm]
        \node (start) [startstop] {FbD foundations};
        \node (pfi) [startstop, left of=start, xshift=-2cm] {Proactive Fairness Integration};
        \node (te) [startstop, below of=start, xshift=-2cm] {Transparency and Explainability};
        \node (uca) [startstop, below of=start, xshift=2cm] {User-Centered Approach};
        \node (cmd) [startstop, right of=start, xshift=2cm] {Continuous Monitoring and Development};
        \draw [arrow] (start) -- (pfi);
        \draw [arrow] (start) -- (te);
        \draw [arrow] (start) -- (uca);
        \draw [arrow] (start) -- (cmd);
    \end{tikzpicture}
    \caption{Principles at the basis of the Fair-by-Design workflow}
    \label{fig:workflow_principles}
\end{figure}
...
\subsubsection{Objective of the workflow application}

At the outset of the Fair-by-Design workflow, a foundational consideration is the precise identification of the objective. This initial step is pivotal in determining the overarching goal of the workflow, laying the groundwork for a comprehensive understanding of whether a classification approach or a regression model is needed.

In this context, the elucidation of the goal involves a detailed examination of the problem domain, aiming to discern the nature of the outcomes sought. This discernment is crucial not only for formulating a clear question about the goals that the workflow is intended to achieve but also for guiding subsequent decisions related to the choice of methodologies, algorithms, and evaluation metrics.

The intricacies of the workflow objective are examined to ascertain whether the study primarily involves categorizing data into distinct classes, indicating a classification task, or predicting numerical values, signifying a regression-oriented inquiry. This nuanced exploration is essential for aligning the approach with the inherent characteristics of the problem at hand.
...
The selection of a fairness notion should align with the specific goals of the workflow application, ethical considerations, and legal requirements.

\subsubsection{Fairness Metric}

A fairness metric quantifies the level of fairness achieved with respect to the chosen fairness notion. The choice of a fairness metric depends on the specific fairness notion and the underlying characteristics of the data. Common fairness metrics include:
...
The alignment between fairness notions and fairness metrics is a critical aspect of the Fair-by-Design workflow, as it establishes a systematic and measurable framework for evaluating fairness. The following considerations guide the selection process with the results also showed in \cref{table:alignment}:
...
\begin{figure}
    ...
    \begin{tikzpicture}[node distance=2cm]
        ...
        \node (dfg) [startstop, below of=fm] {Fairness metric understanding};
        \node (fns) [startstop, below of=dfg] {Aligned Fairness notion choice};
        \node (fms) [startstop, below of=fns] {Consistency assurance};
        \node (dfgfn) [startstop, below of=fn] {Definition of fairness goals};
        \node (fmst) [startstop, below of=dfgfn] {Fairness notion identification};
        \node (end) [startstop, below of=fmst] {Aligned fairness metric choice};
...
\paragraph{Handling Null Values in Discrete Attributes:}
For discrete attributes, such as categorical variables, the strategy involves filling null values with the median of the respective attribute. Mathematically, this operation can be expressed as:

\[ \text{{Fill\_value}}(x) = \text{{median}}(\text{{attribute}}) \]

where \( \text{{Fill\_value}}(x) \) represents the filled value for the null entry in the discrete attribute. Utilizing the median ensures a robust imputation strategy that is less sensitive to outliers compared to using the mean.

\paragraph{Handling Null Values in Continuous Attributes:}
In the case of continuous attributes, such as age or income, null values are filled using the mean of the attribute. The mathematical representation for this process is:

\[ \text{{Fill\_value}}(x) = \text{{mean}}(\text{{attribute}}) \]

Here, \( \text{{Fill\_value}}(x) \) denotes the filled value for the null entry in the continuous attribute. Filling null values with the mean provides a balanced approach, preserving the central tendency of the data.

These strategies for null value handling contribute to the overall robustness of the dataset, ensuring that fillings align with the characteristics of the respective attributes. By incorporating these techniques, the Fair-by-Design workflow maintains the reliability and completeness of the data, essential for training machine learning models that adhere to fairness principles.
