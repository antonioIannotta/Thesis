...
\paragraph{Null values handling:}
For categorical attributes each null values is filled with the median value, while for the continuous attribute each null value is filled with the mean value.
...
\begin{itemize}
    ...
    \item For all other attributes, a random value $\text{random\_value}_{ij}$ is generated, where $j$ represents the specific attribute, and $i$ represents the row being added for that attribute. $\text{random\_value}_{ij}$ is within the most and least represented value for $j$.
        ...
        \For{index \textbf{in} (0, len(combination\_set) - 1)}
            ...
            \While{frequency $<$ max\_frequency}
                ...
                \For{attr \textbf{in} dataset\_attributes \textbf{and not in} protected\_attributes}
                    \State new\_row[attr] $\gets$ random((least\_freq\_combination\_dataset[attr]), (most\_freq\_combination\_dataset[attr]))
                \EndFor
