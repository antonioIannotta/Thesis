...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \Procedure{FairnessUnawarenessProxy}{\textit{dataset}}
        \While{\textbf{not} \textit{dataset\_fair(dataset)} \textbf{or} \textit{protected\_attributes} \textbf{in} \textit{dataset}}
            \State \textit{proxies} $\gets$ \textit{proxy\_detection(dataset)}
            \If{\textit{proxies} \textbf{is empty}}
                \State \textit{dataset} $\gets$ \textit{protected\_attributes\_free\_dataset(dataset)}
            \Else
                \State \textit{dataset} $\gets$ \textit{proxy\_free\_dataset(dataset)}
            \EndIf
        \EndWhile
        \State \textbf{return} \textit{dataset}
        \EndProcedure
    \end{algorithmic}
\end{algorithm}
