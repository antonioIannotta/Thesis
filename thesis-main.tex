...
A deep analysis of the dataset led us to make a first features selection. More specifically for this work only the \emph{important} and \emph{protected} attributes have been selected starting from the whole column set. \\
Another key consideration is about the synonym attributes, the protected attributed reported in the following are selected in order to make to preserve the unicity in the meaning of each one.
...
More specifically:
\begin{itemize}
    \item if the protected attribute is binary no transformation is applied
    \item Otherwise it's transformed into a binary attribute where the value \emph{1} means over the mean and the \emph{0} under the mean.
\end{itemize}
...
To achieve this ambitious objective, a critical aspect of the workflow is the computation of the "level\textunderscore ing" attribute, employing the same pre-processing transformations that have been meticulously applied to the non-binary protected attributes. These transformations serve as the cornerstone for establishing a uniform basis for analysis and prediction. \\
