...
\begin{enumerate}
    ...
    Let \(P\) be the set of protected attributes, and \(p\) represent a specific protected attribute within \(P\). The threshold, denoted as \(t\), is determined for each categorical protected attribute \(p\) in the dataset through the following formalism:

    \[

    t = \frac{\text{min}(p) + \text{max}(p)}{2}
    ...
    \text{Binary Encoding}(p) = \begin{cases} 
