...
In the Fair-by-Design workflow, the traditional data acquisition process is augmented with a deeper focus on the nature of the data, particularly emphasizing the need to identify and understand sensitive attributes. While conventional data acquisition primarily aims at collecting relevant datasets, the Fair-by-Design approach recognizes the necessity of an in-depth exploration of sensitive attributes, pivotal for evaluating fairness.

Understanding these sensitive attributes is critical, as they play a significant role in determining the fairness of the model's outcomes. This knowledge can be obtained through direct communication with stakeholders, as sensitive attributes may sometimes be a deliberate business choice. Alternatively, common-sense reasoning can be applied to identify potential sensitive attributes that might impact fairness.

Upon acquiring the data and obtaining a comprehensive understanding of the sensitive attributes, the next step in the Fair-by-Design workflow involves a fairness evaluation. This evaluation aims to assess the fairness of the dataset concerning these identified sensitive attributes, setting the stage for subsequent model development.

\subsubsection{Fairness Assessment}

The fairness assessment in the Fair-by-Design workflow is an essential checkpoint, ensuring that the dataset is unbiased and aligns with ethical considerations. Disparate Impact, a widely used fairness metric, is employed to quantify and evaluate the fairness of the dataset over the identified sensitive attributes.

Disparate Impact provides a quantitative measure of the disparate treatment of different groups within the dataset. It assesses the ratio of favorable outcomes for different demographic groups, highlighting potential bias and disparities. This metric serves as a crucial indicator during the data acquisition phase, allowing practitioners to make informed decisions regarding fairness considerations before proceeding to subsequent stages of the machine learning pipeline.
...
\begin{itemize}
    \item \textbf{Handling Protected Attributes:} Protected attributes, assumed as categorical attributes in this workflow, undergo a transformation. The most frequent value for each protected attribute is computed, and instances with this value are replaced with 1, while others are replaced with 0. This binary encoding enables the model to recognize instances that are privileged concerning specific attributes.
    
    \item \textbf{Handling Output Variable:} The treatment of the output variable depends on its nature. If categorical, the most frequent value is computed, and instances with this value are replaced with 1, while others are replaced with 0. This transforms the prediction task into determining whether the sample belongs to the most frequent outcome. For continuous output, values above the mean are replaced with 1, and others with 0, converting the prediction task into establishing whether the sample is above the mean.
\end{itemize}
