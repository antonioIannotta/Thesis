...
\begin{abstract}
    ...
    %ABSTRACT 1
    This work presents a series of software methodologies designed with a fair-by-design approach, aimed at addressing the issue of fairness in Artificial Intelligence (AI) systems right from the inception phase. As AI technologies continue to advance and integrate into various aspects of our lives, ensuring fairness and ethical considerations within these systems becomes paramount.   
    The increasing impact of AI on decision-making processes, from credit scoring to job recruitment, highlights the urgency of addressing biases and fairness concerns. Fair-by-design methodologies represent a proactive and preventive approach, emphasizing the incorporation of fairness principles into the very fabric of AI system development.
    This research delves into the conceptualization and implementation of fair-by-design methodologies, exploring techniques to mitigate bias in AI algorithms, datasets, and decision-making processes. By taking fairness into account from the early stages of system design, we aim to reduce the likelihood of perpetuating biases and discrimination.
    The study reviews existing fairness metrics, explores various algorithmic fairness techniques, and examines real-world use cases where fair-by-design methodologies can be applied. By considering diverse perspectives and ethical considerations, this research strives to contribute to the broader goal of creating AI systems that are equitable, transparent, and accountable.
    Key objectives include the development of practical guidelines for software engineers and AI practitioners to incorporate fairness into their projects. By doing so, we can foster the development of AI systems that promote diversity, inclusivity, and ethical decision-making. The ultimate goal is to enable AI systems to benefit society at large while minimizing the risks associated with bias and discrimination.
    In conclusion, this work underscores the importance of fair-by-design software methodologies in the ethical development of AI systems and offers insights into practical approaches for addressing fairness challenges. By placing fairness at the core of AI design, we can make significant strides towards building AI systems that are not only technologically advanced but also socially responsible and just.
    
    %ABSTRACT 2
    %This work presents a comparative analysis of different fair-by-design software approaches, where fair-by-design denotes methods in which fairness is considered from the very inception of the design process. As AI technologies continue to advance and integrate into various aspects of our lives, ensuring fairness and ethical considerations within these systems becomes paramount.
    %The increasing impact of AI on decision-making processes, from credit scoring to job recruitment, highlights the urgency of addressing biases and fairness concerns. Fair-by-design methodologies represent a proactive and preventive approach, emphasizing the incorporation of fairness principles into the very fabric of AI system development.
    %This research delves into the conceptualization and implementation of multiple fair-by-design methodologies, exploring techniques to mitigate bias in AI algorithms, datasets, and decision-making processes. By comparing these methodologies, we aim to identify the strengths and weaknesses of each approach and assess their effectiveness in addressing fairness concerns.
    %The study reviews existing fairness metrics, explores various algorithmic fairness techniques, and examines real-world use cases where fair-by-design methodologies can be applied. By considering diverse perspectives and ethical considerations, this research strives to contribute to the broader goal of creating AI systems that are equitable, transparent, and accountable.
    %Key objectives include the development of practical guidelines for software engineers and AI practitioners to incorporate fairness into their projects based on the insights gained from the comparative analysis. By doing so, we can foster the development of AI systems that promote diversity, inclusivity, and ethical decision-making. The ultimate goal is to enable AI systems to benefit society at large while minimizing the risks associated with bias and discrimination.
    %In conclusion, this work underscores the importance of fair-by-design software methodologies in the ethical development of AI systems and offers insights into practical approaches for addressing fairness challenges through comparative analysis. By placing fairness at the core of AI design and evaluating various methodologies, we can make significant strides towards building AI systems that are not only technologically advanced but also socially responsible and just.
\end{abstract}
...
\include(./sections/state-of-the-art.tex)
...
\section{Fairness through threshold optimizer with equalized odds constraint}
\subsection{Equalized Odds}

Equalized Odds is a fairness metric used to evaluate the disparate impact of a machine learning model's predictions on different demographic groups. It aims to ensure that both true positives and false positives are distributed equitably across these groups.
...
\begin{align*}
A & : \text{Sensitive attribute (e.g., gender, race)} \\
Y & : \text{Binary true label variable (1 for positive, 0 for negative)} \\
\hat{Y} & : \text{Binary predicted label (1 for positive prediction, 0 for negative prediction)}
\end{align*}

The Equalized Odds metric can be formulated as follows:

\subsection{Equalized True Positives Rate (Equalized TPR)}

\[
P(\hat{Y} = 1 | A = a, Y = 1) = P(\hat{Y} = 1 | A = b, Y = 1) \text{ for all } a, b
\]

This ensures that the true positive rate is approximately equal for all sensitive attribute values.

\subsection{Equalized False Positives Rate (Equalized FPR)}

\[
P(\hat{Y} = 1 | A = a, Y = 0) = P(\hat{Y} = 1 | A = b, Y = 0) \text{ for all } a, b
\]

This ensures that the false positive rate is approximately equal for all sensitive attribute values.

\subsection{Threshold adjustment}
n binary classification, the decision threshold is used to determine the predicted class label. By default, the threshold is set at 0.5, meaning that if the predicted probability of the positive class is greater than or equal to 0.5, the sample is classified as positive; otherwise, it's classified as negative.

\subsection{Mathematical Formulation}

Mathematically, the decision threshold can be adjusted as follows:

\begin{align*}
\text{Lowering the threshold:} & \quad \text{Predict positive class if } P(\hat{Y}=1) \geq \text{Threshold}, \\
\text{Raising the threshold:} & \quad \text{Predict positive class if } P(\hat{Y}=1) < \text{Threshold}.
\end{align*}

Adjusting the threshold (\text{Threshold}) changes the classification rule and impacts performance metrics such as precision and recall.
...
\section{Fairness through threshold optimizer with equalized odds constraint}
To accomplish the results outlined in the abstract, we leveraged the \emph{ThresholdOptimizer} class from the \emph{FairLearn} library, a powerful tool for mitigating bias and achieving fairness in machine learning models. In particular, we utilized the \emph{Equalized Odds} constraint, a key fairness objective, to ensure that the distribution of false positives and false negatives was balanced across different demographic groups.\\
The `ThresholdOptimizer` class allowed us to fine-tune the decision threshold for model predictions, ensuring that fairness constraints were met. By systematically adjusting the classification threshold, we optimized the trade-off between precision and recall while adhering to the Equalized Odds fairness metric.
%----------------------------------------------------------------------------------------
...
\begin{tabular}{|c|c|}
    ...
    \hline
    RandomForest Classifier  &  10, gini, 10, 10\\
    \hline
    XGBoost Classifier & 5, 1.5, 1.0, 0.8, 5\\
    \hline
...
\begin{tabular}{|c|c|}
    ...
    \hline
    RandomForest Classifier  &  0.9738398544131028\\
    \hline
    XGBoost Classifier &  0.9741810737033667\\
    \hline
    DecisionTree Classifier &  0.9740673339399454\\
    \hline
\end{tabular}
...
\begin{tabular}{|c|c|}
    ...
    RandomForest Classifier  &  10, gini, 10, 10\\
    \hline
    XGBoost Classifier & 1, 0.5, 1.0, 1.0, 4\\
    \hline
    DecisionTree Classifier &  entropy, 10, 10\\
    \hline
\end{tabular}
...
\begin{tabular}{|c|c|}
    ...
    RandomForest Classifier  &  0.9738398544131028\\
    \hline
    XGBoost Classifier & 0.9738398544131028\\
    \hline
    DecisionTree Classifier &  0.9738398544131028\\
    \hline
\end{tabular}
...
\subsection{Fairness through threshold optimizer with equalized odds constraint}
\subsubsection{Best models}
\begin{tabular}{|c|c|}
    \hline
    \textbf{Model} & \textbf{Best parameters} \\
